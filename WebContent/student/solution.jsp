<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="java.util.*" import="cn.justquiet.Beans.Student" import="cn.justquiet.Beans.Question" import="cn.justquiet.Beans.Answer"%>
...
%>
	<div id="personal-msg">
		<div class="personal-title"><h4>${qst.title }</h4>
		</div><dl>
			<dt><a href="javascript:void(0)">${qst.nickname }</a></dt>
			<dd>等级：</dd>
			<dd><span>${qst.slevel }</span></dd>
		</dl>
	</div>
	<div id="paper-sheet">
		<div class="paper-sheet-code">
		<p>${qst.codes }</p>
		</div><div class="paper-sheet-illustration">
		<p>${qst.details }</p>
		</div>
	</div>
	<div id="count-answer">
...
<%	
		if(count>0){
			...
			if(listans!=null){
				Iterator itans = listans.iterator();
			for(int i=1;itans.hasNext();i++){
				pageContext.setAttribute("ans",itans.next());
%>
		<div class="one-of-those">
			<dl><dd>#<span><%=i %></span>楼&nbsp;</dd><dd><a href="javascript:void(0)" title="等级：${ans.level }">${ans.nickname }</a></dd><dd>&nbsp;@${ans.date }</dd></dl>
			<div>${ans.answer }</div>
			<dl class="support-or-oppose"><dt><a href="javascript:void(0)">赞成(<span>${ans.support }</span>)</a></dt><dt><a href="javascript:void(0)">反对(<span>${ans.against }</span>)</a></dt></dl>
		</div><!-- 赞成或反对数超过四位数会溢出，凡是如此，则 统一变成+999-->
<%
			}
		}
	}
%>
	...
	<form action="../handleStuAnswer?index=<%=index %>&qid=${ans.qid }" method="post">
		<div class="answer-content"><textarea name="answer" placeholder="我的见解"></textarea></div>
