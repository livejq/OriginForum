<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="java.util.*" import="cn.justquiet.Beans.Teacher" import="cn.justquiet.Beans.Student" import="cn.justquiet.Beans.Task" import="cn.justquiet.Beans.Check" import="cn.justquiet.ServiceImpl.GatherBusinessImpl"%>
...
<%
		...
		List<Check> done = gbi.QueryCheckByTkcodes(tk.getTkcodes(),1);
		List<Student> notdo = gbi.QueryCheckNotDone(tk.getTkcodes(), 0);
		int allmember = gbi.QueryClassMember(tk.getCid());
%>
	<div id="showContent"><!-- 此模块为点击未完成的作业码显示的页面信息 -->
		    <div class="task-title">
		      ...
		      <div class="panel">
		          ...
		          <table id="send-list" cellspacing="0" cellpadding="0">
		            <thead>
		            <tr>
			            <th width="90"><label><input type="checkbox"> 全选</label></th>
			            <th width="110">学号</th>
...
%>
		            <tr>
		            	<td class="choose"><input type="checkbox" value="${itdone.path }/${itdone.stkname}" name="<%=i %>"></td>
...
<%
			...
			int allmember = gbi.QueryClassMember(tk.getCid());
	%>
		<div id="showContent"><!-- 此模块为点击进入已完成作业码时显示的页面信息 -->
			    <div class="task-title">
			    	<h4>${ftkok.tktitle }</h4>
			    	<span></span>
			    </div>
...
<%		
		...
		if(listnot.size()!=0){
			GatherBusinessImpl gbi = new GatherBusinessImpl();
			List<Check> done = gbi.QueryCheckByTkcodes(listnot.get(0).getTkcodes(),1);
			String cutoff = gbi.QueryCutoff(listnot.get(0).getTkcodes());
			List<Student> notdostu = gbi.QueryCheckNotDone(listnot.get(0).getTkcodes(), 0);
			int allmember = gbi.QueryClassMember(listnot.get(0).getCid());
...
%>
	<div id="showContent"><!-- 此模块为点击进入（未传入参数时所默认显示的未完成页面信息） -->
		    <div class="task-title">
		    	<h4>${ftknot.tktitle }</h4>
			<%
			if(done.size()>0){
